...
Click a node to select it and open its editor. The default editor shows only the thought, Save and a **Details** disclosure. **Details** reveals kind and status; **Notes & sources** and connection settings expand separately. Existing metadata is retained when editing only the label. The editor starts closed. On desktop it is a compact popover beside the selected card; opening it does not refit the tree. On small screens it is a bounded bottom sheet. Close × and Escape retain a dirty draft visibly; a successful save closes the editor and restores card focus. Context is available through the compact disclosure beneath the strategic question. Save changes explicitly. Pan the blank canvas by dragging; scroll to zoom; use Reset view and Expand all. Reset view keeps a readable minimum scale and centres the active branch when the whole tree would become too small; pan or deliberately zoom out for a wider overview. The editor stays anchored during pan, zoom and resize; its height is bounded, and expanded details scroll. Narrow previews preserve a readable automatic minimum scale. Each card’s **+** opens a short contextual child menu on mouse hover or click. Hover preserves selection and keyboard focus; click keeps the menu open. Choosing a kind opens an unsaved child draft beneath the invoking card. Choices respect the current method and do not change it. Arrow keys navigate; Enter chooses; Escape or an outside click dismisses. The **A** shortcut bypasses the menu using the default suggestion. A branch change invalidates an open menu. A separate chevron expands or collapses existing children. Keyboard users can tab to nodes and press Enter/Space to edit, and use the separately labelled Add, Tree type and collapse controls; Left/Right also collapse/expand. The active branch and its ancestor path are highlighted. Reduced-motion preferences disable visual animations.
