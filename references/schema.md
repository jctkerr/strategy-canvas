...
A small source badge on a card opens **Source peek** inside the card's existing notes panel. LinkedIn-only references retain the LinkedIn icon. Web-only or mixed-provider references use a globe, with a tooltip identifying the provider mix; the chooser also labels each source LinkedIn or Web. The badge appears only for explicitly attached structured records or references on observed descendants; legacy source text is not automatically classified. On a collapsed family or method, the count deduplicates both LinkedIn and web sources from observed descendants. The count means attached examples, not successful or validated methods. Proposed/catalogue descendants do not inflate ancestor counts merely because they cite a source. Their own attached references remain available while their provenance stays explicit.

The peek shows the original source link, author/title, inspection date, recorded observations and limitations, and the citing thought's provenance and notes under **Why this thought cites it**. When opened from an ancestor it identifies the observed descendants that supply the reference. Multiple sources use a compact chooser. The editor's **Sources** disclosure contains the source/basis text and **View linked sources** when structured references exist; web links remain available there too.
...
Opening a source keeps the editor and its draft in the same panel. **Back** or Escape clears the frame and returns to notes, restoring the editing position. Opening another card's source follows the normal save/conflict guard before switching cards. Selection/focus/state changes reconcile removed or hidden source nodes; removing the current source clears its iframe and selects another available record without automatically loading it. Peek state is local to the browser and is never stored in canonical state.
...
### Recent browser selection

For a request such as “explore this”, read the live view context:

```sh
python3 /absolute/path/strategy-canvas/scripts/canvas.py \
  --session /absolute/path/to/session focus
```

`GET /api/view` returns the same `{status, stateRevision, selection}`. Browser hints live in the session's ephemeral `view.json`; they never increment the canonical strategy revision. `status` is `current`, `missing`, `stale` or `ambiguous`. Only a current, unambiguous selection can resolve the request. Otherwise inspect a fresh preview or clarify which card; do not guess from an old tab. Selection provides context, not authority to edit.

A current `selection` contains `selectedNodeId`, `focusedNodeId`, `editingNodeId`, `hasUnsavedDraft`, `observedRevision`, `ageSeconds`, the canonical `node`, its `effectiveMethod`, root-to-parent `ancestors`, `descendantNodeIds` and directly referenced `sources`. Node and source content comes from current canonical state, never an editor draft. `observedRevision` may lag `stateRevision`. If `hasUnsavedDraft` is true, preserve the pending human work; do not overwrite that node.

The browser sends `POST /api/view` with JSON fields `clientId`, `sequence`, `selectedNodeId`, `focusedNodeId`, `editingNodeId`, `hasUnsavedDraft`, `revision` and optional `active` (default `true`). Node IDs must exist or be `null`; `sequence` and `revision` are positive integers, and the observed revision cannot exceed canonical state. Older or repeated per-client sequences are ignored. Active hints expire after 90 seconds without a heartbeat. Different active `(selectedNodeId, focusedNodeId)` pairs return `ambiguous` with `candidateNodeIds`; otherwise the most recent hint is used. This is transient live-session context, not chat synchronisation or an exported part of the tree.
...
Click a card or its notes icon to open a right-docked panel on desktop or a bounded bottom sheet on narrow screens. The thought and **Notes** are visible directly; **Sources** and **Details** hold secondary fields. Existing metadata is retained when editing only the label. Existing-card edits save automatically after a short pause, with save status and errors visible. The panel stays open after saving; switching cards or closing first finishes the pending save. New children remain drafts until **Add**. An invalid, conflicting or failed draft stays available instead of being silently discarded. Pan and zoom do not refit the tree when the panel opens; long panel content scrolls. Reset view keeps a readable minimum scale and centres the active branch when the whole tree would become too small.

Each card's **+** opens a suggested child draft immediately; hover does not change selection or create a draft. **Add as** offers suitable alternative kinds without replacing authored text or changing the parent method. **Add** creates the node; discard creates nothing. **A** opens the same draft. The separate chevron collapses/expands children. Tree method badges appear at the root and explicit subtree overrides; inherited choices remain reachable through **Details** or **T**. Enter/Space edits a focused card; Left/Right collapse or expand. Controls retain keyboard access and typing is not intercepted. Reduced-motion preferences are respected.

Select any non-root thought and use **Focus branch** in the existing canvas controls to view that thought and its descendants as a readable subtree. On entry, the view reveals two levels beneath the selected thought; deeper parent nodes stay collapsed until expanded or focused in turn. While focused, a small breadcrumb shows its label and **← Whole tree**; use that return action to restore the wider map and its prior selection, collapsed branches and pan/zoom. Focus branch can deepen again from another selected descendant. Expand all expands only the current focused subtree, preserving other branches' collapse state. Let pending card edits finish saving before changing focus; add or discard a new-child draft first. Maps with more than 24 thoughts or six leaf thoughts initially show the root and collapsed top-level families; smaller maps initially show every branch. This is progressive disclosure of the complete state. Focus is browser view state: it does not change canonical content, node IDs, parent links, sources or siblings. Live sessions share an ephemeral selection hint with the agent; a reload starts with the whole tree. If the focused node is removed, or the selected thought moves outside it during a canonical update, the view returns to the whole tree. Editing and adding deeper thoughts still update the same canonical map.
...
The compact **Problem** control opens the optional situation, desired change and constraints brief. A card’s **Tree type** opens a nonmodal menu anchored beside it. Choosing one of nine methods saves immediately and closes the menu; there is no separate apply step. Escape, clicking outside or moving keyboard focus outside dismisses without saving. Explicitly typed cards keep their chip visible; inherited cards expose it on selection. Arrow keys move between choices without saving; Enter or Space chooses. **Example & help** keeps the full explanation, source, limits and mixed-method guidance optional; scope and inheritance remain available there. The type chip on each explicitly typed card marks a subtree starting point. Types belong to branches, not global depth levels. If another editor changes the selected method or its ancestor chain while the picker is open, applying is blocked until **Review latest setting**; unrelated note edits remain usable and are preserved. The contextual **Add** control names a useful next contribution for that approach and opens a draft under the selected node; only **Add** creates it in the canonical tree. Its suggested kind and relation remain editable. Delete branch removes the selected node and its descendants; the root cannot be deleted. Undo reverses the most recent locally saved action; successive autosaves in one card-editing visit form one undo group. An intervening external revision disables it. This is one-level undo, not a full history. View selection, pan/zoom and collapsed branches are local to the browser and do not change canonical JSON.

When another editor updates the session, the canvas polls and redraws while preserving selection and view. An unsaved local draft is retained; changing its underlying node or child-draft parent shows a conflict notice and blocks saving stale fields. Copy any wanted draft text, discard and reopen against the latest node before reconciling. Unrelated edits remain mergeable. Switching cards finishes pending autosave first and stops if a conflict or error remains. New-child drafts require Add or discard, so entered work is not silently lost. This save protection concerns edits only; conversational prompts are always optional.

When browser storage is available, recovery retains standalone state and pending card drafts locally; live sessions keep canonical state on the server and recover browser drafts against it. Recovery is scoped to browser origin and canvas identity. Each document has a distinct draft record; same-tab reload or back/forward navigation can recover its previous draft. New tabs load saved canonical state without adopting another tab’s pending work. New canvases and separate export artifacts have separate identities. Restored drafts keep their conflict checks. Unreadable stored records are preserved and failures are shown. This is not cloud or device sync. Export remains the portable backup and sharing route; recovery does not rewrite an HTML file or sync a standalone canvas to its original session.
...
Normally, let card edits finish saving; add or discard new-card drafts and resolve conflicts before export. The menu refreshes canonical state. If browser storage fails, JSON, HTML, SVG and Markdown can instead include the valid current edited-card snapshot while preserving the dirty editor and original canonical record. This fallback does not mark the edit saved or overwrite unreadable storage. Print directs the user to a file export. Other unresolved drafts still block export. All formats include every branch, even when the view has collapsed branches.

- **Interactive canvas:** standalone `.html` containing the current state, styles and scripts. It works without the server, including editing, local browser recovery when storage is available, and further export. Edits do not sync back to the original session or change the downloaded HTML file.
...
Letter shortcuts apply only when an SVG card itself has focus: **A** opens a child draft; **Shift+A** opens a sibling draft; **Enter/E** edits; **T** opens its type menu. Left collapses expanded children, otherwise moves to the parent; Right expands collapsed children, otherwise moves to the first child. Up/Down move between siblings. Tab remains native focus navigation. **Cmd/Ctrl+Enter** saves a card immediately, adds a new card or submits a changed question editor. Escape returns from sources to notes, or finishes a pending existing-card save before closing the panel. An unresolved or new-card draft stays available until saved, added or explicitly discarded. Menu buttons keep their own keyboard behaviour.
