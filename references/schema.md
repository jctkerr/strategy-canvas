...
Each card's **+** opens a short contextual child menu on hover or click. Hover preserves selection and keyboard focus; click keeps the menu open. Choosing a kind opens a draft under that card, preserving its tree method. Arrow keys navigate; Enter chooses; Escape or an outside click dismisses. **A** opens the suggested child draft directly. A branch change invalidates an open menu. A separate chevron expands or collapses children. Nodes and their Add, Tree type, notes and collapse controls have keyboard access; Enter/Space edits a focused card, and Left/Right collapse or expand. The active branch and ancestor path are highlighted. Reduced-motion preferences disable visual animations.
