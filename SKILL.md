...
Orient a new user in one or two sentences: read left to right, click a thought to edit it and use its **+** to add beneath it; keep the conversation in their agent. Point to **How to use** for the controls rather than reciting a manual or requiring a framework choice. Explain the current save mode: live edits save to the session, while standalone edits need another export. The skippable introduction is optional browser guidance, not required analysis or a reason to alter the tree.
