...
Orient a new user in one or two sentences: edit the main question beside its heading, click a thought to edit it and use **+** to choose what to add beneath it. Hover or click reveals a short menu suited to that card; selecting a choice opens a blank draft and preserves the tree type. **A** still adds the suggested default directly. They may work manually or keep discussing the tree in their agent. The compact editor leads with the thought itself and returns to the tree after saving. Card kind and status stay under **Details**, with notes and sources in a further optional disclosure. The canvas suggests a child kind and may mark a suitable tree type; these are editable defaults, not semantic judgements or automatic framework changes. Mention **A**, **Enter/E**, **T** and **Cmd/Ctrl+Enter** only if keyboard use would help; shortcuts apply to focused cards and editors. Point to **How to use** for the controls rather than reciting a manual or requiring a framework choice. Explain the current save mode: live edits save to the session, while standalone edits need another export. The skippable introduction is optional browser guidance, not required analysis or a reason to alter the tree. On the blank starter, **Go straight to canvas** opens an editable root immediately; the person can add their question later.
...
If someone starts with the blank standalone canvas or **New**, use their exported JSON to establish a live session. That separate tab does not sync to the old session: do not read the old `/api/state` and claim it contains their new question. Preserve their manual branches and ask only for material gaps.
