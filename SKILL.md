...
description: Explore and develop strategic possibilities conversationally in an editable left-to-right tree, focus on useful branches, and compare or decide when ready. Use when someone wants to brainstorm and visualise strategy together in a persistent animated preview, then export the reasoning.
---
...
- Make scoped MECE a standing requirement: before presenting a decomposition and after every update, apply the method reference's check. State one parent question/universe, one grouping basis and each sibling's inclusion/exclusion boundary; check overlaps and coverage. Regroup mixed dimensions such as audience, channel, format, status and evidence rather than silently treating them as peer options. Resolve gaps or visibly mark the specific uncertainty. Distinguish partitions from sequence, causal, support and reference links; not every tree edge is a partition. Keep one canonical item and primary classification home, using references or secondary annotations for cross-cutting relationships and counting shared evidence once. Neither an Other/hybrid bucket nor schema validation proves semantic completeness.
