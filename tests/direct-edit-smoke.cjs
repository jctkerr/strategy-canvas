...
const choices=p=>p.locator('#draft-choice option');
async function chooseDefault(p,id){await card(p,id).locator('.node-add').click();await p.locator('#label').waitFor({state:'visible'});}
...
(async()=>{
  ...
  try {
    ...
    assert.equal(await page.locator('#add-menu').isHidden(),true,'Hover never opens an intermediate menu');
    assert.equal(await card(page,'b').locator('.node-add').getAttribute('aria-haspopup'),null);
    assert.equal(await card(page,'b').locator('.node-add').getAttribute('aria-controls'),'edit-form');
    ...
    assert.ok(issueChoices.length>=2&&issueChoices.length<=5,'Keep a short list of appropriate draft alternatives');
    assert.ok(issueChoices.every(label=>label.trim().length>0&&label.trim().split(/\s+/).length<=10),'Choice labels are short and readable');
    assert.equal(await page.locator('dialog[open], [aria-modal="true"]:visible').count(),0,'The draft stays nonmodal');
    assert.equal(await page.locator('#label').evaluate(el=>el===document.activeElement),true,'One click focuses writing');
    await page.screenshot({path:path.join(output,'direct-add-desktop.png')});
    await page.locator('#discard').click();await closeEditor(page);
    assert.deepEqual(await read(),renamed,'Discarding the draft writes nothing');
    await chooseDefault(page,'child');
    const driverChoices=await choices(page).allTextContents();
    assert.notDeepEqual(driverChoices,issueChoices,'A metric in a driver branch gets different alternatives from an issue question');
    assert.equal(await page.locator('#label').evaluate(el=>el===document.activeElement),true,'A numerical draft also focuses writing');
    await page.keyboard.press('Escape');
    assert.equal(await page.locator('#label').isVisible(),true,'Escape keeps an unsaved draft');
    assert.deepEqual(await read(),renamed,'Opening and Escape never change a framework or state');
    await page.locator('#discard').click();await closeEditor(page);
    assert.equal(await card(page,'child').evaluate(el=>el===document.activeElement),true,'Closing returns focus to the owning card');
    checks.push('Hover is passive; + opens a focused nonmodal draft with short contextual alternatives, while Escape preserves drafts and discard writes nothing.');

    await chooseDefault(page,'b');
    assert.equal(await page.locator('#add-menu').isHidden(),true,'The direct draft has no intermediate menu');
    ...
    assert.equal((await read()).revision,renamed.revision,'Opening Add creates a draft only');
    ...
    assert.equal(node.method,undefined,'Adding a child never automatically applies a new tree type');
    ...
    checks.push('The suggested draft belongs to its clicked parent; adding preserves frameworks, while the separate chevron only collapses or expands.');

    await card(page,'b').locator('.node-add').focus(); await page.keyboard.press('Enter');
    assert.equal(await page.locator('#label').evaluate(el=>el===document.activeElement),true,'Enter on + goes straight to writing');
    await page.locator('#draft-choice').focus();
    const alternative=await choices(page).nth(1).getAttribute('value');
    await page.locator('#draft-choice').selectOption(alternative);
    assert.equal(await page.locator('#kind').inputValue(),alternative.split(':')[0],'The optional type control changes draft classification');
    await page.locator('#draft-choice').selectOption(await choices(page).first().getAttribute('value'));
    ...
    assert.equal(await page.locator('#add-menu').isHidden(),true,'A dirty draft never exposes an intermediate selection menu');
    ...
    await page.locator('#discard').click(); await closeEditor(page);
    checks.push('Keyboard Enter on + and A both start writing; draft alternatives are optional, and another card cannot steal an unsaved draft or change its parent.');

    await card(page,'a').click(); await card(page,'a').focus();await page.keyboard.press('t');
    ...
    const anchor=await card(page,'a').boundingBox();
    ...
    assert.equal(await card(page,'a').evaluate(el=>el===document.activeElement),true,'Escape returns focus to the branch control');
    assert.deepEqual(await read(),beforeToggle,'Escape must not save any change');
    await card(page,'a').focus();await page.keyboard.press('t');
    ...
    let releaseResponse, responseHeld, pendingResponse;
    ...
    try {
      ...
      const response=pendingResponse=page.waitForResponse(r=>r.url().endsWith('/api/state')&&r.request().method()==='PUT');
      response.catch(()=>{}); // Preserve the first assertion if the pending request never arrives.
      ...
      // The anchored picker can cover a sibling; keyboard access still exercises the busy guard.
      await card(page,'b').focus();await page.keyboard.press('Enter');
      await card(page,'root').locator('.node-add').focus();await page.keyboard.press('Enter');
      ...
      releaseResponse();
      if(pendingResponse)await pendingResponse.catch(()=>{});
    ...
    const mobileDraft=await phone.locator('#draft-type-row').boundingBox();
    assert.ok(mobileDraft.x>=0&&mobileDraft.y>=0&&mobileDraft.x+mobileDraft.width<=391&&mobileDraft.y+mobileDraft.height<=845,'Draft alternatives fit a phone-width viewport');
    const bounds=await phone.locator('#draft-choice').boundingBox();assert.ok(bounds.height>=30,'The optional type selector remains touchable');
    assert.ok((await choices(phone).allTextContents()).every(label=>label.trim()),'Every alternative has a visible label');
    await phone.screenshot({path:path.join(output,'direct-add-mobile.png')});
    assert.equal(await phone.locator('#label').isVisible(),true,'One tap opens writing without needing hover');
    assert.equal(await phone.locator('#label').evaluate(el=>el===document.activeElement),true,'Touch focuses the draft text');
    ...
    await page.locator('#draft-choice').selectOption({label:'Driver'});
    ...
    checks.push('Solution draft alternatives add qualitative Driver cards; A adds a proposed solution, Undo preserves the tree, exploration suggests Solution, and numerical Driver subtrees still add metrics.');
