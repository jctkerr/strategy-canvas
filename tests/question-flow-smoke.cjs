...
(async()=>{
  ...
  try{
    ...
    assert.equal(await page.locator('#save').evaluate(el=>el===document.activeElement),true,'Native Tab still advances through the form');
    ...
    checks.push('E/Enter edits, T chooses a tree, modified Enter saves, and Escape retains dirty text while closing clean editors; text entry and Tab keep native behavior.');

    before=await fresh(page);
    await focusCard(page,'metric');await page.keyboard.press('a');
    assert.equal(await page.locator('#kind').inputValue(),'metric');
    assert.equal(await page.locator('#quick-kind').isVisible(),true,'The suggested card kind is editable without expanding Details');
    assert.equal(await page.locator('#quick-kind').inputValue(),'metric');
    ...
    await page.keyboard.press('Escape');await isFocused(page,child.id);
    ...
    await page.locator('#quick-kind').selectOption('evidence');
    assert.equal(await page.locator('#kind').inputValue(),'evidence','The quick selector updates the saved card-kind field');
