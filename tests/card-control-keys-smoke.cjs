...
(async()=>{
 ...
 try{
  ...
  await page.locator('#label').fill('Investigate delivery');const added=await saved(page,()=>page.locator('#label').press('Control+Enter'));await page.locator('.inspector').waitFor({state:'hidden'});assert.equal(added.nodes.length,2);assert.equal(await page.locator('#node-count').textContent(),'2 thoughts');const child=added.nodes.find(n=>n.id!=='root');assert.equal(child.parentId,'root');assert.equal(added.nodes[0].method,'issue');assert.equal(added.nodes[0].notes,fixture.nodes[0].notes);
  ...
  const plus=card(page,child.id).locator('.node-add');await plus.focus();await page.keyboard.press('Enter');await page.locator('#add-menu').waitFor({state:'visible'});assert.equal(await page.locator('.inspector').isHidden(),true);assert.deepEqual(await read(),added);await page.keyboard.press('Escape');
  assert.equal(await plus.evaluate(el=>el===document.activeElement),true);await page.keyboard.press('Space');await page.locator('#add-menu').waitFor({state:'visible'});await page.keyboard.press('Escape');
  await page.keyboard.press('ArrowLeft');assert.equal(await plus.evaluate(el=>el===document.activeElement),true,'Arrows on the plus retain the existing control behavior');assert.deepEqual(await read(),added);
  checks.push('Enter and Space on + still open its choices, Escape restores control focus, and arrow keys do not become card-navigation commands.');
  ...
  await card(page,child.id).locator('.node-add').focus();await page.keyboard.press('e');await page.locator('#label').fill('AET is ordinary draft text');await page.locator('#label').press('Enter');assert.equal(await page.locator('#label').inputValue(),'AET is ordinary draft text\n');assert.equal(await page.locator('#approach-panel').isHidden(),true);await card(page,'root').locator('.node-type').focus();await page.keyboard.press('a');assert.equal(await page.locator('#label').inputValue(),'AET is ordinary draft text\n');assert.deepEqual(await read(),added,'Card shortcuts must preserve an existing unsaved draft');await discard(page);
  checks.push('Typing shortcut letters and Enter inside the editor stays ordinary text; using another card control while a draft exists cannot replace or save it.');
